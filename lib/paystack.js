...
export async function initializeTransaction({ email, amountNaira, reference, metadata, plan, callbackUrl }) {
  const body = {
    ...
    currency: 'NGN', // explicit on purpose — without this, Paystack falls back to the account's default currency, which can silently end up being USD and break NGN-only methods like Zap and bank transfer entirely
    reference,
    metadata,
  };
...
// Larger transfers can come back requiring an OTP sent to your business
// phone/email before Paystack will actually release the money — this
// completes that step once you have the code. (Paystack also lets you
// disable OTP for transfers entirely under Settings → Preferences, if
// you'd rather skip this altogether.)
export async function finalizeTransfer({ transferCode, otp }) {
  ...
  const data = await res.json();
  if (!data.status) throw new Error(data.message || 'Could not finalize transfer with that OTP');
  return data.data;
}
